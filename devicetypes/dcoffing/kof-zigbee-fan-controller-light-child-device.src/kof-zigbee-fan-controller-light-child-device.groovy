...
metadata {
	...
	tiles(scale: 2) {
         standardTile("switch", "switch", decoration: "flat", width: 6, height: 4, canChangeIcon: true) {    		
        	state "off", label:"OFF", action: "on", icon: iconURL+"light_grey.png", backgroundColor: "#ffffff", nextState: "turningOn"
			state "on", label: "ON", action: "off", icon: iconURL+"lightH.png", backgroundColor: "#00A0DC", nextState: "turningOff"
            state "turningOn", label:"TURNING ON", action: "on", icon: iconURL+"lightI.png", backgroundColor: "#2179b8", nextState: "turningOn"
            state "turningOff", label:"TURNING OFF", action:"off", icon: iconURL+"lightI.png", backgroundColor:"#2179b8", nextState: "turningOff"
        }    	
